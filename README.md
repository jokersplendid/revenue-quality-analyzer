...
我做了一个收入质量分析 MVP，把传统收入底稿里的人工复核改造成可复用的数据分析流程。它不是只抽凭证，而是先识别收入模型，再根据不同收入模型选择业务驱动因子和复算公式。

以基金手续费收入为例，申购费和赎回费看交易金额和费率，管理费看平均 AUM、年费率和计费天数，业绩报酬看超额收益和计提门槛。基于这些模型，我对收入记录做复算，并把费率偏差、期末集中确认、低回款、期后退款、新客户大额、关联方等信号转成可解释评分，最后输出高风险样本池和后续追问清单。

更偏数据分析的部分是：我增加了收入质量分层、产品画像、增长质量归因和 LLM 洞察生成。这样报告不只是说“哪些样本有风险”，还能解释“收入增长的质量怎么样、是否可持续、下一步应该验证什么”。
